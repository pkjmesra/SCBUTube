...
/**
 * Convenience trace macro. Depends on LOG_VERBOSE.
 * 
 * For example: DDLogTrace() -> @"MyVC: viewDidLoad"
 **/

//#define DDLogTrace() do{ if(LOG_VERBOSE) NSLog(@"%@: %@", THIS_FILE, THIS_METHOD); }while(0)
#define DDLogTrace() do{ if(LOG_VERBOSE) NSLog(@"%s",__PRETTY_FUNCTION__); }while(0)
