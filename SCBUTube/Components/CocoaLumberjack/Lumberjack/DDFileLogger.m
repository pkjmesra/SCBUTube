...
- (BOOL)isLogFile:(NSString *)fileName
{
	...
	{
		NSCharacterSet *hexSet = [NSCharacterSet characterSetWithCharactersInString:@"0123456789ABCDEF-_"];
...
/**
 * Generates a timestamp in yyyy-MM-dd_HH-mm-ss format for use in the log file's name.
 **/
...
- (NSString *)createNewLogFile
{
	...
	do
	{
		NSString *fileName = [NSString stringWithFormat:@"log-%@.txt", [self currentTimeStamp]];//[self generateShortUUID]];
...
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage
{
    ...
    if ((logMessage->logLevel & LOG_FLAG_WARN) ==LOG_FLAG_WARN)
    {
		// All above warning levels - includes - error, warnings, info and verbose
